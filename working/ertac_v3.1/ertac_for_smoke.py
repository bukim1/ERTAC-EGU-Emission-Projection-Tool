...
import sys
try:
    import getopt, logging, os, time, re, csv, datetime, io, calendar
except ImportError:
...
Usage: %s [OPTION]...
  ...
  --suppress-pdw                        suppress warnings about the percentage distribution not being equal to 100 percent if it is within 1 percent
...
def fix_inputs(conn, inputvars, logfile):
    ...
    print("  Fixing Missing Information in Inputs", file=logfile)
    
    
    logging.info("      Deleting units that have not yet come online or are retired")         
    print("      Deleting units that have not yet come online or are retired", file=logfile)
    ...
    print("      Inserting units into PUSP that are missing", file=logfile)
    ...
    print("      Filling in missing PUSP identifiers", file=logfile)
    ...
    print("      Replacing blank offline start dates", file=logfile)
    ...
    print("      Filling in missing stack info", file=logfile)
    ...
    if (inputvars['output_type'] == 'ORL') and (not inputvars['notz']):    
        ...
        print("      Filling in missing time zone info", file=logfile)
        for results in conn.execute("""SELECT plant_latitude, plant_longitude, cuuaf.orispl_code, cuuaf.unitid 
            ...
            try:
                ...
                print("  Time zone looked up for "+results[2]+"/"+results[3]+" found to be " + tz, file=logfile)
            except (geonames.GeonamesError, KeyError, httplib.BadStatusLine) as err:
...
def process_unit_level_ers(conn, inputvars, logfile):
    ...
    #it should be noted that this algorithm will not properly handle changes in emission rates that occur during a period of time and only one of those rates will be properly selected
    
    logging.info("  Loading Default Emission Rates")
    print("  Loading Default Emission Rates", file=logfile)
    ...
    print("  Processing Unit Level Emission Rates", file=logfile)
...
def process_results(conn, inputvars, logfile):
    ...
    for (state, fuel_unit_type_bin) in conn.execute(query, inputs).fetchall():
        ...
        print("Processing - " + state + ", " + fuel_unit_type_bin, file=logfile)
        ...
        print("  Calculating Emissions", file=logfile)
        ...
        print("  Converting Files to SMOKE Ready", file=logfile)
...
def run_diagnostics(conn, inputvars, logfile):
    ...
    print("Running Diangostics", file=logfile)
...
def check_uaf_consistency(conn, inputvars, logfile):
    missing_location_info = conn.execute("""SELECT orispl_code, unitid, state, camd_by_hourly_data_type FROM calc_updated_uaf WHERE offline_start_date >= ? AND (fips_code IS NULL OR plant_latitude IS NULL OR plant_longitude IS NULL) ORDER BY state""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()

    if len(missing_location_info) > 0:
        print("Warning:", len(missing_location_info), "facility/units in UAF are missing lat/long information necessary for SMOKE processing:", file=logfile)
...
def check_pusp_info_file_consistency(conn, inputvars, logfile):
    ...
    if len(non_unique_pusp_info_file) > 0:
        print("Warning:", len(non_unique_pusp_info_file), "plant/unit/stack/processes in PUSP Info have more than one entry:", file=logfile)
    ...
    EXCEPT SELECT orispl_code, unitid, ertac_fuel_unit_type_bin FROM ertac_pusp_info_file """, (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()

    if len(unit_uaf_not_pusp_info_file) > 0:
        print("Warning:", len(unit_uaf_not_pusp_info_file), "facility/units in UAF did not match any ORISPL_CODE, UNITID, ERTAC_FUEL_UNIT_BIN_TYPE in PUSP Info File:", file=logfile)
        for unit in unit_uaf_not_pusp_info_file:
            ...
            print("  " + ertac_lib.nice_str(result), file=logfile)

    unit_pusp_info_file_not_uaf = conn.execute("""SELECT orispl_code, unitid, ertac_fuel_unit_type_bin FROM (SELECT * FROM ertac_pusp_info_file WHERE offline_start_date >= ?) 
    EXCEPT SELECT orispl_code, unitid, ertac_fuel_unit_type_bin FROM calc_updated_uaf""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()

    if len(unit_pusp_info_file_not_uaf) > 0:
        print("Warning:", len(unit_pusp_info_file_not_uaf), "facility/units in ertac_pusp_info_file did not match any ORISPL_CODE, UNITID, ERTAC_FUEL_UNIT_BIN_TYPE in UAF:", file=logfile)
    ...
    for (column) in ['stkhgt', 'stkdiam', 'stktemp', 'stkflow', 'stkvel', 'scc']:    
        result = conn.execute("""SELECT orispl_code, unitid, state, plantid, pointid, stackid, segment  FROM ertac_pusp_info_file WHERE """+column+""" IS NULL AND offline_start_date >= ? ORDER BY state""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()
        if len(result) > 0:
            print("Warning:", len(result), "facility/units were missing "+column+" and will use default", file=logfile)
            for unit in result:
                print("  " + ertac_lib.nice_str(unit), file=logfile)
                
    for (column) in ['stkhgt', 'stkdiam', 'stktemp', 'stkflow', 'stkvel']:    
        result = conn.execute("""SELECT orispl_code, unitid, state, plantid, pointid, stackid, segment  FROM ertac_pusp_info_file WHERE """+column+""" IS NOT NULL AND """+column+""" = 0 AND offline_start_date >= ? ORDER BY state""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()
        if len(result) > 0:
            print("Warning:", len(result), "facility/units have data in "+column+" that resembles fugitive emission traits", file=logfile)
            for unit in result:
                print("  " + ertac_lib.nice_str(unit), file=logfile)             
    ...
    for (polcode, column) in [['NOX', 'nox'],['SO2', 'so2']]+ppollutants:    
        if polcode not in inputvars['pollutants']:   
            if not polcode.isdigit(): 
                if inputvars['suppress_pdw']:
                    result = conn.execute("""SELECT SUM(COALESCE("""+column+"""_percentage,0)) as total, orispl_code, unitid FROM ertac_pusp_info_file WHERE offline_start_date >= ? GROUP BY orispl_code, unitid, ertac_fuel_unit_type_bin HAVING (SUM(COALESCE("""+column+"""_percentage,0)) >= 101.0 or SUM(COALESCE("""+column+"""_percentage,0)) <= 99.0) ORDER BY state""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()
                else:
                    result = conn.execute("""SELECT SUM(COALESCE("""+column+"""_percentage,0)) as total, orispl_code, unitid FROM ertac_pusp_info_file WHERE offline_start_date >= ? GROUP BY orispl_code, unitid, ertac_fuel_unit_type_bin HAVING SUM(COALESCE("""+column+"""_percentage,0)) != 100.0 ORDER BY state""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()
                if len(result) > 0:
                    if inputvars['suppress_pdw']:
                        print("Warning:", len(result), "facility/units in had a percentage distribution for "+polcode+" emissions that did summed to 101 or more or 99 or less:", file=logfile)
                    else:
                        print("Warning:", len(result), "facility/units in had a percentage distribution for "+polcode+" emissions that did not sum to 100:", file=logfile)
                    for unit in result:
                        if unit[0] != 100:
                            ...
                            print("  " + ertac_lib.nice_str(result2), file=logfile)  
...
def check_additional_control_consistency(conn, inputvars, logfile):
    ...
    unit_additional_control_not_uaf = conn.execute("""SELECT orispl_code, unitid 
    FROM ertac_base_year_rates_and_additional_controls 
    WHERE factor_end_date >= ?
    EXCEPT SELECT orispl_code, unitid FROM calc_updated_uaf""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()

    if len(unit_additional_control_not_uaf) > 0:
        print("Warning:", len(unit_additional_control_not_uaf), "facility/units in additional control did not match any ORISPL_CODE, UNITID in UAF:", file=logfile)
        for unit in unit_additional_control_not_uaf:
            print("  " + ertac_lib.nice_str(unit), file=logfile)

    unit_additional_control_not_uaf = conn.execute("""SELECT orispl_code, unitid FROM (SELECT * FROM calc_updated_uaf WHERE offline_start_date >= ? ORDER BY state) 
    EXCEPT SELECT orispl_code, unitid FROM ertac_base_year_rates_and_additional_controls""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()

    if len(unit_additional_control_not_uaf) > 0:
        print("Warning:", len(unit_additional_control_not_uaf), "facility/units in UAF have no control entries:", file=logfile)
        for unit in unit_additional_control_not_uaf:
            print("  " + ertac_lib.nice_str(unit), file=logfile)
    ...
    AND factor_end_date >= ?
    ORDER BY orispl_code, unitid, pollutant_code, factor_start_date, factor_end_date""",(ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()
    ...
    HAVING COUNT(*) > 1
    ORDER BY orispl_code, unitid, pollutant_code""", (ertac_lib.first_day_of(inputvars['future_year']),)).fetchall()
        ...
        for (next_start, next_end) in factors.fetchall():
            if prev_end is None or next_start is None or prev_end >= next_start:
                if not heading_printed:
                    print("Warning: control/emissions has factors with missing or overlapping start/end dates:", file=logfile)
                    heading_printed = True
            if prev_end is None:
                print("  " + ertac_lib.nice_str((plant, unit, poll, prev_start, prev_end)) + " missing end date", file=logfile)
            if next_start is None:
                print("  " + ertac_lib.nice_str((plant, unit, poll, next_start, next_end)) + " missing start date", file=logfile)
            if prev_end >= next_start:
                print("  " + ertac_lib.nice_str((plant, unit, poll)) + ": " + ertac_lib.nice_str((prev_start, prev_end)) + " overlaps " + ertac_lib.nice_str((next_start, next_end)), file=logfile)
    ...
    if len(factors_past_dates) > 0:
        print("Warning: control/emissions has factor_start_date missing, before, or during base year; will be ignored:", file=logfile)
        for dates in factors_past_dates:
            print("  " + ertac_lib.nice_str(dates), file=logfile)

def check_additional_variables_consistency(conn, inputvars, logfile):
    print("Checking additional variables.", file=logfile)
    ...
    if len(state_missing) > 0:
        print("Warning:", len(state_missing), "states/ertac fuel unit type bin combination in UAF did not match any state in additional variables:", file=logfile)
        for state in state_missing:
            print("  " + ertac_lib.nice_str(state), file=logfile)

def qa_results(conn, inputvars, logfile):
    logging.info("Running QA Tests")   
    print("Running QA Tests", file=logfile)
    if inputvars['output_type'] == 'ORL':
        #TODO
        x = 1
    else:
        result = conn.execute("""SELECT cas, SUM(ann_emis) FROM ff10_future GROUP BY cas""")
        print("Displaying Annual Emissions By Pollutant", file=logfile)
        for row in result:
            print("  " + ertac_lib.nice_str(row), file=logfile)
        ...
        print("Displaying Annual Emissions By Region", file=logfile)
        for row in result:
            print("  " + ertac_lib.nice_str(row), file=logfile)
        ...
        print("Displaying Sum of Hourly Emissions By Pollutant, County", file=logfile)
        for row in result:
            print("  " + ertac_lib.nice_str(row), file=logfile)             
...
def create_annual_summary_file(conn, inputvars, logfile):
    ...
    conn.execute("""INSERT INTO annual_summary_with_other_pollutants(orispl_code, unitid, facility_name, state, fips_code, ertac_region, ertac_fuel_unit_type_bin, by_ertac_fuel_unit_type_bin, max_unit_heat_input, ertac_heat_rate, generation_capacity, nameplate_capacity, fy_op_hours, fy_op_hours_max, by_uf, fy_uf, by_gload, by_heat_input, fy_gload, fy_heat_input, by_so2_mass, by_so2_rate, by_nox_mass, by_nox_rate, by_os_nox_mass, by_os_nox_rate, by_os_heat_input, by_os_gload, by_non_os_nox_mass, by_non_os_nox_rate, fy_so2_mass, fy_so2_rate, fy_so2_max, fy_nox_mass, fy_nox_rate, fy_nox_max, fy_os_nox_mass, fy_os_nox_rate, fy_os_heat_input, fy_os_gload, fy_non_os_nox_mass, fy_non_os_nox_rate, hierarchy_order, longitude, latitude, gdu_flag, retirement_date, new_unit_flag, data_type, program_codes, 
...
def write_final_data(conn, inputvars, input_prefix_fs, out_prefix, produce_diagnostics, produce_annual, produce_hourly, logfile):
    ...
    if produce_diagnostics:
        ...
        header = ["#FORMAT=FF10_POINT", 
                  ...
                  "#PROJECTION YEAR     "+inputvars['future_year'], 
                  "#TYPE     Point Sources", 
                  "#DESC     ANNUAL", 
                  "#DESC     Contact: ertacegu@gmail.com", 
                  "#DESC     ERTAC Version: "+VERSION,
                  "#DESC     ERTAC to SMOKE Run Date: " + time.strftime("%m/%d/%y"), 
                  "#DESC     Emissions, in short tons, from ERTAC"]
        
        print(file=logfile)
        if input_prefix_fs is None:
            csv_file = "ertac_additional_smoke_headers.csv"
        else:
            csv_file = input_prefix_fs + "ertac_additional_smoke_headers.csv"
        try:
            cf = open(csv_file, 'r')
            for ha in csv.reader(cf): 
                if ha[0][:6] != "#DESC ":
                    print("Error: line in header additions not properly formatted by beginning with '#DESC ' and will not be included: "+ha[0], file=logfile)
                else: 
                    header = header + ha
...
def main(argv=None):
    ...
    try:
        opts, args = getopt.getopt(argv[1:], "hdqv:o:",
            ...
            "state=", "ignore-pollutants=", "input-type=", "output-type=", "run-qa", "suppress-pdw", "monthly", "runtz"])
    ...
    for opt, arg in opts:
        ...
        if opt in ("-h", "--help"):
            ...
            inputvars['run_qa'] = True
        elif opt in ("--suppress-pdw"):
            inputvars['suppress_pdw'] = True
    ...
    for file_name in [os.path.basename(sys.argv[0]), 'ertac_lib.py', 'ertac_tables.py', 'ertac_reports.py',
        ...
        print("  " + file_name + ": " + time.ctime(os.path.getmtime(os.path.join(sys.path[0], file_name))), file=logfile)
    ...
    if len(inputvars['pollutants']) > 0:
        ...
        print("Ignoring Pollutants: "+",".join(inputvars['pollutants']), file=logfile)
    ...
    for rpo in rpos: 
        if (inputvars['output_type'] == 'ORL'):
            ...
            dbconn.execute("""DELETE FROM pt_hourly_future""")
        else:
            dbconn.execute("""DELETE FROM ff10_future""")
            dbconn.execute("""DELETE FROM ff10_hourly_future""")
        if rpo[1]:
        ...
        if 'state' in inputvars:
            ...
            print("Proccessing States: "+inputvars['state'], file=logfile)
        ...
        for inputvars['month'] in range(1,13):
            ...
            inputvars['fy_end_date'] = inputvars['future_year']+"-"+m+"-"+str(calendar.monthrange(int(inputvars['base_year']),    inputvars['month'])[1])
            ...
            print(file=logfile)
            print("Summarizing data for Month: "+m, file=logfile)
            ...
            if inputvars['monthly']:
                ...
                write_final_data(dbconn, inputvars, input_prefix_fs, new_new_output_prefix, False, False, True, logfile)

  
        dbconn.execute("""UPDATE ff10_future SET ann_emis = """ + " + ".join([m+"_value" for m in month_names]))    
        logging.info("Writing out reports:")
        write_final_data(dbconn, inputvars, input_prefix_fs, new_output_prefix,False,True, (not inputvars['monthly']), logfile)
    
    if inputvars['input_prefix_pp'] is not None:    
        logging.info("Creating Annual Summary File")
        create_annual_summary_file(dbconn, inputvars, logfile)   
              
    write_final_data(dbconn, inputvars, input_prefix_fs, output_prefix, True,False,False, logfile)
