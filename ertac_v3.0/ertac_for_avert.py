...
import sys
try:
    import getopt, logging, os, time, re, csv, datetime, io, http.client, calendar
except ImportError:
...
def export_array_to_csv(array_data, prefix, basic_csv_file, connection, column_types, logfile):
    ...
    print(file=logfile)
    try:
        #correct csv write to be python 3 compatible
    ...
    print("Wrote out", row_count, "data rows from array to file: " + csv_file, file=logfile)
...
def process_results(conn, inputvars, out_prefix, logfile):
    ...
    print("  Converting Files to AVERT Ready", file=logfile)
...
ORDER BY cuuaf.orispl_code, cuuaf.unitid, cuuaf.ertac_fuel_unit_type_bin""", (inputvars['future_year'], inputvars['future_year']+'-01-01'))
    ...
    "avert_hourly_nox.csv"]:
        ...
        try:
            #Correct csv write to be python 3 compatible
            writers[file_counts] = csv.writer(open(csv_file, 'w', newline=''))
        except IOError:
            print("File: " + csv_file + " -- Could not be written.", file=logfile)
            return
        file_counts+=1
    ...
    if unit_rows == 0:    
        ...
        print("No units found in files.  Exiting.", file=logfile)
        return False
