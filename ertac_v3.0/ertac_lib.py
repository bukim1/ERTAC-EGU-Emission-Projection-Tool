...
try:
    ...
    try:
        ...
        sys.stderr.write("Fatal error: can't import all required modules.")
        #print ("No SQLite3 available with this Python.", file == sys.stderr)
        sys.stderr.write("No SQLite3 available with this Python.")
        raise
...
def load_csv_into_table(prefix, basic_csv_file, table_name, connection, column_types, logfile, delete_old_rows=True):
    ...
    if column_count != len(column_types):
        #print ("Programmer error: Number of columns in table " + table_name, file == sys.stderr)
        sys.stderr.write("Programmer error: Number of columns in table " + table_name)
        #print ("created by SQL script does not match ertac_tables.py list of column types.", file == sys.stderr)
        sys.stderr.write("created by SQL script does not match ertac_tables.py list of column types.")
        sys.exit(1)
...
def compute_proxy_generation(connection, region, fuel, plant, unit, state, name, base_year, future_year, logfile):
    ...
    if new_old_ratio is not None and new_old_ratio > 0.0 and old_optimal_load is not None and old_optimal_load > 0.0:
    ...
    ORDER BY temporal_allocation_order""", (region, fuel)).fetchall():
        ...
        if future_date >= unit_online and future_date < unit_offline:
            ...
            if fuel.upper() == 'COAL':
                ...
                if unit_max_hi is not None and unit_max_hi > 0.0 and unit_heat_rate is not None and unit_heat_rate > 0.0 and proxy_percentage is not None and proxy_percentage > 0.0:
                    gload_proxy = unit_max_hi * 1000.0 / unit_heat_rate * proxy_percentage / 100.0

            elif new_old_ratio is not None and new_old_ratio > 0.0:
